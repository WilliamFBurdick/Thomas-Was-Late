...
SoundManager::SoundManager()
{
	...
	//loop all fire sound when played
...
void SoundManager::playFire(Vector2f emitterLocation, Vector2f listenerLocation)
{
	...
	case 1:
		...
		if (m_Fire1Sound.getStatus() == Sound::Status::Stopped) m_Fire1Sound.play();
		break;
	case 2:
		//locate/move the source of the sound
		m_Fire2Sound.setPosition(emitterLocation.x, emitterLocation.y, 0.0f);
		if (m_Fire2Sound.getStatus() == Sound::Status::Stopped) m_Fire2Sound.play();
		break;
	case 3:
		//locate/move the source of the sound
		m_Fire3Sound.setPosition(emitterLocation.x, emitterLocation.y, 0.0f);
		if (m_Fire3Sound.getStatus() == Sound::Status::Stopped) m_Fire3Sound.play();
		break;
	}
